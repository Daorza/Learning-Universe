...
<title>Material Detail Data | Edit</title>
        ...
        <div class="flex justify-start mx-auto ml-32">
            ...
            <div class="flex-1 p-2 shadow-md bg-white rounded-md mt-10 w-4/5 mx-auto">
                <div class="relative overflow-x-auto sm:rounded-lg px-20">
                    ...
                    <form action="{{ route('detail-material.update', ['class_id' => $onlineClass->id, 'material_id' => $material_id, 'id' => $detail->id]) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="space-y-2">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="detail_title" id="detail_title" required value="{{  old('detail_title', $detail->detail_title) }}" required
                                class="block w-full p-2 mt-2 shadow rounded-md text-gray-900 bg-stone-50 appearance-none focus:outline-gray-800 focus:bg-gray-100 focus:shadow-inner">
                        </div>
                        <br>
                        <div class="space-y-2">
                            <label for="detail_subtitle" class="font-medium">Subtitle</label>
                            <input type="text" name="detail_subtitle" id="detail_subtitle" required value="{{ old('detail_subtitle', $detail->detail_subtitle) }}" required
                                class="block w-full p-2 mt-2 shadow rounded-md text-gray-900 bg-stone-50 appearance-none focus:outline-gray-800 focus:bg-gray-100 focus:shadow-inner">
                        </div>
                        <br>
                        <div class="space-y-2">
                            <label for="detail_text" class="font-medium">Text</label>
                            <input type="text" name="detail_text" id="detail_text" required value="{{ old('detail_text', $detail->detail_text) }}" required
                                class="block w-full p-2 mt-2 shadow rounded-md text-gray-900 bg-stone-50 appearance-none focus:outline-gray-800 focus:bg-gray-100 focus:shadow-inner">
                        </div>
                        <br>
